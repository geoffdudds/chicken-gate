...
from ..shared.config import CAMERA_IP, CAMERA_PASSWORD, CAMERA_USERNAME
...
def camera_snapshot():
    ...
    try:
        ...
        print(
            "OpenCV not available (normal on Pi Zero) - using placeholder with RTSP info"
        )
    except Exception as e:
...
def create_rtsp_info_image():
    ...
    try:
        ...
        img = Image.new(
            "RGB", (width, height), color="#27ae60"
        )  # Green background since RTSP works
        ...
        line_height = 35
        total_height = (
            len([line for line in lines if line]) * line_height
        )  # Exclude empty lines
    ...
    try:
        ...
        return Response(
            "Camera not accessible via HTTP - Tapo cameras require special protocols",
            mimetype="text/plain",
        )
...
def camera_stream():
    ...
    try:
        ...
        return Response(
            "Camera streaming not available on Pi Zero - Use RTSP URL in VLC",
            mimetype="text/plain",
        )
    ...
    try:
        ...
        for port in key_ports:
            ...
            try:
                ...
                test_result["content_type"] = response.headers.get(
...
if __name__ == "__main__":
    ...
    if port == 80:
        ...
        print(
            f"Running on port {port} (development mode) - web interface will be available at:"
        )
